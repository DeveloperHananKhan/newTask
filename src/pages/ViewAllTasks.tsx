...
export const ViewAllTask = () => {
  ...
  const totalTask = task.map((t) => t);
  const totalTaskLength = totalTask.length;
  const tasksPerPage = 5;
  const [currentPage, setCurrentPage] = useState(1);
  const firstIndex = (currentPage - 1) * tasksPerPage;
  const lastIndex = currentPage * tasksPerPage - 1;
  const currentTasks = task.slice(firstIndex, lastIndex + 1);
  const totalPages = Math.ceil(totalTaskLength / tasksPerPage);
  ...
  return (
    <>
            ...
            >
              <div className="grid grid-cols-2 items-center gap-3 lg:flex lg:justify-around lg:items-center">
                <div>
                  ...
                  <h3 className="font-semi-bold text-gray-700">
                    {t.startTime}
                  </h3>
                </div>
                <div>
                ...
                <div className="flex lg:flex gap-2 relative top-2">
                  ...
                  <div>
                    ...
                    <button
                      className="cursor-pointer"
                      onClick={() => {
                        setCurrentTask(t), setIsTaskOpen(true);
                      }}
                    >
                      {" "}
                      <svg
              ...
              <div className=" flex justify-self-center mt-4 ">
                <div className="flex gap-2 items-center">
                  ...
                  <h3 className="font-semi-bold text-gray-700 text-sm">
                    {t.discription}
                  </h3>
                </div>
              </div>
      ...
      <div className={`flex justify-center gap-2 ${currentPage > 1 ? "block" : 'hidden'}`}>
        {arrayPages.length > 1 && currentPage < totalPages &&  (
          <button className="border-1 bg-gray-300 text-white rounded px-2" onClick={() => setCurrentPage(currentPage + 1)}>
            {" "}
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="20"
        ...
        {arrayPages.map((item, index) => (
          <div key={index}>
            <button
              className={`${
                currentPage === item ? "bg-[#4B7BE5] text-white" : "bg-white"
              } px-3 py-1 rounded`}
              value={currentPage}
        ...
        <button className={`border-1 bg-gray-300 text-white rounded px-2 ${currentPage === 1 ? 'hidden' : 'block'}`}    onClick={() => setCurrentPage(currentPage - 1)}>
          {" "}
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="20"
